...
group.add_argument("-i", "--init", help="init Hydrokit",
                   action='store_true', required=False)
parser.add_argument("-p", "--ph-test", help="pH test",
                    action='store_true', required=False)
parser.add_argument("-o", "--orp-test", help="ORP test",
                    action='store_true', required=False)
parser.add_argument("-e", "--ec-test", help="EC test",
                    action='store_true', required=False)
parser.add_argument("-t", "--temp-test", help="temp test",
                    action='store_true', required=False)
parser.add_argument("-c", "--count", help="test loop count",
...
def read_and_validate(val, test_val, error_margin, adc):
    ...
    for _ in range(0, TEST_RUN_COUNT):
        adc_reading = MCP342x.convert_and_read(
            adc, aggregate=numpy.mean, samples=ADC_SAMPLES)
...
def read_calib_data_from_eeprom(val):
    val_bytearray = eeprom.extension_data_read(
        struct.calcsize('<f'), val["eeprom_calib_addr"])
...
def hydrokit_init():
 # check if extension is HydroKit available in the bus
    ...
    print("=============== " + EC_STR +
          " Factory Calibration ==================")
    print(bcolors.WARNING + "Disconnect any probe from the " +
          EC_STR + bcolors.WARNING + " terminal!" + bcolors.ENDC)
    ...
    print("\n====================== " + EC_STR +
          " Testing =======================")
    print(bcolors.WARNING + "Connect 1K Resistor test probe to " +
          EC_STR + bcolors.WARNING + " terminal!" + bcolors.ENDC)
    ...
    if ec["error_count"] > 0:
        print("Wrong test " + EC_STR +
              " value! Faulty hardware, test probe not attached or wrong test probe!")
        return 1
    else:
        print("EC 1K Resistor test probe test: " +
              bcolors.OKGREEN + "Success!\n" + bcolors.ENDC)

    # Test EC with Shunt probe
    print(bcolors.WARNING + "Connect Shunt test probe to " +
          EC_STR + bcolors.WARNING + " terminal!" + bcolors.ENDC)
    input("Press Enter when ready to proceed with the test...")
    ...
    if ec["error_count"] > 0:
        ...
        print("EC Shunt test probe test: " +
              bcolors.OKGREEN + "Success!\n" + bcolors.ENDC)
    ...
    for _ in range(0, TEST_RUN_COUNT):
        ...
        r = MCP342x.convert_and_read_many(
            adcs, samples=ADC_SAMPLES, aggregate=numpy.mean)
    ...
    if ec["error_count"] > 0:
        print("Wrong test " + EC_STR +
              " value, faulty hardware or test probe not attached!")
        retval = False
    ...
    if retval:
        ...
        if (float(ph_offset_eeprom) == float(ph["calib_data"])) and \
            ...
            print("HydroKit factory calibration: " +
                  bcolors.OKGREEN + "Success!\n" + bcolors.ENDC)
        else:
            print("HydroKit factory calibration: " + bcolors.FAIL +
                  "FAILED when writting to EEPROM!\n" + bcolors.ENDC)
            retval = False
...
def main():
    ...
    if args.ph_test:
        ...
        for c in range(0, TEST_RUN_COUNT):
            ph["val"] = MCP342x.convert_and_read(
                adc_ph, aggregate=numpy.mean, samples=ADC_SAMPLES)
            ph_v = ph["val"] - 1.250 - ph_calib_data
            ph_val = 7 - (- ph_v*1000 / standard_slope)
            print(ph["name"] + "(RAW V): " + str(ph["val"]) + " | " + ph["name"] +
                  "(V): " + str(ph_v) + " | " + ph["name"] + "(pH Val): " + str(ph_val))
    ...
    if args.orp_test:
        ...
        for c in range(0, TEST_RUN_COUNT):
            orp["val"] = MCP342x.convert_and_read(
                adc_orp, aggregate=numpy.mean, samples=ADC_SAMPLES)
            orp_v = orp["val"] - 1.250 - orp_calib_data
            print(orp["name"] + "(RAW V): " + str(orp["val"]) +
                  " | " + orp["name"] + "(V): " + str(orp_v))
    ...
    if args.ec_test:
        ...
        k = 1.0
        for c in range(0, TEST_RUN_COUNT):
            ec["val"] = MCP342x.convert_and_read(
                adc_ec, aggregate=numpy.mean, samples=ADC_SAMPLES)
            ec_v = ec["val"]
            gain = ec_v / ec_calib_data
            resistance = 1000 / (gain - 1)
            uS = 1000000 * k / resistance
            print(ec["name"] + "(RAW V): " + str(ec["val"]) + " | " + ec["name"] +
                  "(V): " + str(ec_v) + " | " + ec["name"] + "(uS): " + str(uS))
    ...
    if args.temp_test:
        ...
        for c in range(0, TEST_RUN_COUNT):
            rtd["val"] = MCP342x.convert_and_read(
                adc_rtd, aggregate=numpy.mean, samples=ADC_SAMPLES)

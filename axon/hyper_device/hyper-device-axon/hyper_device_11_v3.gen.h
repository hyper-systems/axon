...
#ifndef __HYPER_DEVICE_CLASS_11__
#define __HYPER_DEVICE_CLASS_11__
...
void hyper_device_11_reset(hyper_device_11_t * device) {
...
// --- PRINT DEVICE ---
void hyper_device_11_print(hyper_device_11_t * device) {
  HYPER_PRINTF(
    "(hyper_device_11_t\n"
    "  (version %d)\n"
    "  (device_class_id %lu)\n"
  ...
  if (device->atmospheric_pressure_set) {
    HYPER_PRINTF("  (atmospheric_pressure (f32 %f))\n", device->atmospheric_pressure);
  } else {
    HYPER_PRINTF("  (atmospheric_pressure (f32 none))\n");
  }
...
hyper_result_t hyper_device_11_set_firmware_version(hyper_device_11_t * device, uint8_t * value, uint8_t value_size) {
...
// --- ENCODE DEVICE ---
hyper_result_t hyper_device_11_encode(hyper_device_11_t * device, uint8_t * out, uint8_t * out_len) {
  cmp_ctx_t cmp_ctx = {0};
  ...
  if (device->atmospheric_pressure_set) ++defined_attributes_count;
...
// --- DECODE DEVICE ---
hyper_result_t hyper_device_11_decode(hyper_device_11_t * device, uint8_t * in, uint8_t in_size) {
  cmp_ctx_t cmp_ctx = {0};
...
// --- DISPATCH DEVICE ---
uint8_t hyper_device_11_dispatch(hyper_device_11_t * device) {
  uint8_t count = 0;
...
#endif // __HYPER_DEVICE_CLASS_11__

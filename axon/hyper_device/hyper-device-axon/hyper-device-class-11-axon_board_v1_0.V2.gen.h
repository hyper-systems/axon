...
#ifndef __HYPER_DEVICE_CLASS_11_H__
#define __HYPER_DEVICE_CLASS_11_H__
...
#define HYPER_DEVICE_11_ATTRIBUTES_COUNT 8
...
typedef struct {
  ...
  float humidity;
  uint8_t atmospheric_pressure_set : 1;
...
typedef struct {
  uint8_t *buf;
  size_t offset;
} hyper_device_11_msgpack_t;

static bool hyper_device_11_msgpack_reader(cmp_ctx_t *ctx, void *data, size_t count) {
  hyper_device_11_msgpack_t *msgpack = (hyper_device_11_msgpack_t *)ctx->buf;
  memcpy(data, msgpack->buf + msgpack->offset, count);
  msgpack->offset += count;
  return true;
}

static size_t hyper_device_11_msgpack_writer(cmp_ctx_t *ctx, const void *data, size_t count) {
  hyper_device_11_msgpack_t *msgpack = (hyper_device_11_msgpack_t *)ctx->buf;
...
static void hyper_device_11_error_and_exit(const char *msg) {
    fprintf(stderr, "%s\n\n", msg);
    exit(EXIT_FAILURE);
}


void hyper_device_11_init(hyper_device_11_t * device, uint8_t device_id[6]) {
  device->version = 2;
  device->device_class_id = 11;
  ...
  device->humidity = 0.0;
  device->atmospheric_pressure_set = 0;
  device->atmospheric_pressure = 0.0;
...
void hyper_device_11_pp(hyper_device_11_t * device) {

      printf("(hyper_device_11_t\n"
              "  (version %d)\n"
              "  (device_class_id %lu)\n"
      ...
      if (device->atmospheric_pressure_set) {
        printf("  (atmospheric_pressure (f32 %f))\n", device->atmospheric_pressure);
      } else {
        printf("  (atmospheric_pressure (f32 none))\n");
      }
...
void hyper_device_11_bind_reboot(hyper_device_11_t * device, void (*callback)(bool)) {
  device->on_reboot_update = callback;
}


void hyper_device_11_bind_publish_interval(hyper_device_11_t * device, void (*callback)(uint16_t)) {
...
void hyper_device_11_set_environment_temperature(hyper_device_11_t * device, float value) {
  device->environment_temperature_set = 1;
  device->environment_temperature = value;
}


void hyper_device_11_set_humidity(hyper_device_11_t * device, float value) {
  device->humidity_set = 1;
  device->humidity = value;
}


void hyper_device_11_set_atmospheric_pressure(hyper_device_11_t * device, float value) {
  device->atmospheric_pressure_set = 1;
  device->atmospheric_pressure = value;
}


void hyper_device_11_set_illuminance(hyper_device_11_t * device, float value) {
  device->illuminance_set = 1;
  device->illuminance = value;
}


void hyper_device_11_set_firmware_version(hyper_device_11_t * device, uint8_t * value, uint8_t value_size) {
  device->firmware_version_size = value_size;
  device->firmware_version = value;
}


void hyper_device_11_set_reboot(hyper_device_11_t * device, bool value) {
  device->reboot_set = 1;
  device->reboot = value;
}


void hyper_device_11_set_uptime(hyper_device_11_t * device, uint64_t value) {
  device->uptime_set = 1;
  device->uptime = value;
}


void hyper_device_11_set_publish_interval(hyper_device_11_t * device, uint16_t value) {
...
uint8_t hyper_device_11_encode(hyper_device_11_t * device, uint8_t * out) {
  cmp_ctx_t cmp_ctx = {0};
  hyper_device_11_msgpack_t msgpack = {out, 0};

  cmp_init(&cmp_ctx, (void *)&msgpack, NULL, NULL, hyper_device_11_msgpack_writer);

  // Encode message as array of elements.
  if (!cmp_write_array(&cmp_ctx, 3)) {
    hyper_device_11_error_and_exit(cmp_strerror(&cmp_ctx));
  }
  ...
  // Attributes
  if (!cmp_write_array(&cmp_ctx, HYPER_DEVICE_11_ATTRIBUTES_COUNT)) {
    hyper_device_11_error_and_exit(cmp_strerror(&cmp_ctx));
  }
  ...
  if (device->atmospheric_pressure_set) {
    if (!cmp_write_float(&cmp_ctx, device->atmospheric_pressure)) {
      hyper_device_11_error_and_exit(cmp_strerror(&cmp_ctx));
    }
  } else {
    if (!cmp_write_nil(&cmp_ctx)) {
...
uint8_t hyper_device_11_decode(hyper_device_11_t * device, uint8_t * in, uint8_t in_size) {
  cmp_ctx_t cmp_ctx = {0};
  hyper_device_11_msgpack_t msgpack = {in, 0};

  cmp_init(&cmp_ctx, (void *)&msgpack, hyper_device_11_msgpack_reader, NULL, NULL);
  ...
  if (!cmp_read_object(&cmp_ctx, &obj)) {
    hyper_device_11_error_and_exit("could not read object");
  }
  if (!cmp_object_as_uint(&obj, &device->device_class_id)) {
    hyper_device_11_error_and_exit("could not read device_class_id\n");
  }
  ...
  if (device_id_size != 6) {
    hyper_device_11_error_and_exit("hyper_device_decode: wrong device_id_size\n");
  }
  ...
  if (!cmp_read_array(&cmp_ctx, &attributes_array_size) && attributes_array_size != HYPER_DEVICE_11_ATTRIBUTES_COUNT) {
    hyper_device_11_error_and_exit(cmp_strerror(&cmp_ctx));
  }
  ...
  if (!cmp_object_is_nil(&obj)) {
    if (cmp_object_as_float(&obj, &device->environment_temperature)) {
      ...
      hyper_device_11_error_and_exit("could not read value\n");
    }
  }
  ...
  if (!cmp_object_is_nil(&obj)) {
    if (cmp_object_as_float(&obj, &device->atmospheric_pressure)) {
      device->atmospheric_pressure_set = 1; 
    } else {
      hyper_device_11_error_and_exit("could not read value\n");
    }
...
void hyper_device_11_dispatch(hyper_device_11_t * device) {

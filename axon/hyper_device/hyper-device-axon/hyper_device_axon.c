...
static void hyper_device_axon_set_data(uint8_t *data, uint8_t data_len){
    ...
    hyper_device_11_decode(&hyper_device, data, data_len);
    hyper_device_11_dispatch(&hyper_device);
	LOG_INF("after update:");
	hyper_device_11_pp(&hyper_device);
#else
    hyper_device_12_decode(&hyper_device, data, data_len);
    hyper_device_12_dispatch(&hyper_device);
	LOG_INF("after update:");
	hyper_device_12_pp(&hyper_device);
#endif
}
...
static uint8_t hyper_device_axon_get_data(uint8_t *data){
    ...
    hyper_device_11_set_firmware_version(&hyper_device, HYPER_FIRMWARE_VER, sizeof(HYPER_FIRMWARE_VER));
    ...
    // [SEND] Encode device.
	uint8_t message_len = hyper_device_11_encode(&hyper_device, data);
#else
    ...
    hyper_device_12_set_firmware_version(&hyper_device, HYPER_FIRMWARE_VER, sizeof(HYPER_FIRMWARE_VER));
    ...
    // [SEND] Encode device.
	uint8_t message_len = hyper_device_12_encode(&hyper_device, data);
#endif

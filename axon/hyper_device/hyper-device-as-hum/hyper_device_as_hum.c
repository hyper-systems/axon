...
static int hyper_device_as_ezo_hum_read(float *temp, float *hum, float *dew)
{
	...
	hyper_hexdump(hum_data, sizeof(hum_data));
	if (!strcmp("*WARM", hum_data))
	{
		LOG_ERR("EZO-HUM: warming up, cannot take reading at the moment!");
		return -1;
	}
	...
	nul_ptr = strtok(NULL, ",");
	dew_ptr = strtok(NULL, ",");
	// LOG_INF("tmp: %s | hum: %s | dew: %s\n", temp_ptr, hum_ptr, dew_ptr);

	// Convert string to float
	*temp = atof(temp_ptr);
	*hum = atof(hum_ptr);
	*dew = atof(dew_ptr);
	// LOG_INF("tmp: %f | hum: %f | dew: %f\n", *temp, *hum, *dew);

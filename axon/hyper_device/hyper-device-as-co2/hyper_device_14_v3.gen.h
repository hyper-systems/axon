...
#ifndef __HYPER_DEVICE_CLASS_14__
#define __HYPER_DEVICE_CLASS_14__
...
void hyper_device_14_reset(hyper_device_14_t * device) {
  device->co2_set = 0;
  device->co2 = 0;
}


void hyper_device_14_init(hyper_device_14_t * device, uint8_t device_id[6]) {
  device->version = 3;
  device->device_class_id = 14;
  memcpy(device->device_id, device_id, 6);
  hyper_device_14_reset(device);
}


// --- PRINT DEVICE ---
void hyper_device_14_print(hyper_device_14_t * device) {
  HYPER_PRINTF(
    "(hyper_device_14_t\n"
    "  (version %d)\n"
    "  (device_class_id %lu)\n"
  ...
  if (device->co2_set) {
    HYPER_PRINTF("  (co2 (u16 %u))\n", device->co2);
  } else {
    HYPER_PRINTF("  (co2 (u16 none))\n");
  }
  HYPER_PRINTF(")\n");
}
...
// --- SET ATTRIBUTES ---
void hyper_device_14_set_co2(hyper_device_14_t * device, uint16_t value) {
  device->co2_set = 1;
  device->co2 = value;
}
...
hyper_result_t hyper_device_14_encode(hyper_device_14_t * device, uint8_t * out, uint8_t * out_len) {
  ...
  if (device->co2_set) ++defined_attributes_count;
  ...
  if (device->co2_set) {
    ...
    if (!cmp_write_u16(&cmp_ctx, device->co2)) {
      HYPER_PRINTF("%s\n", cmp_strerror(&cmp_ctx));
      return HYPER_ERR_ENCODE;
    }
  }
...
hyper_result_t hyper_device_14_decode(hyper_device_14_t * device, uint8_t * in, uint8_t in_size) {
  ...
  for (uint16_t i = 0; i < attributes_map_len; ++i) {
    ...
    if (cmp_object_as_uchar(&obj, &key)) {
      ...
      switch (key) {
      case 0: // co2
        if (cmp_object_as_ushort(&obj, &device->co2)) {
          device->co2_set = 1; 
        } else {
...
// --- DISPATCH DEVICE ---
uint8_t hyper_device_14_dispatch(hyper_device_14_t * device) {
  uint8_t count = 0;
...
#endif // __HYPER_DEVICE_CLASS_14__

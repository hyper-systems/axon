 ...
 * Copyright (c) 2021 Hyper Systems
...
uint16_t modes_flags[] = {
    VEML7700_ALS_CONF_ALS_IT_800_ms | VEML7700_ALS_CONF_ALS_GAIN_x2,   // (0)
    VEML7700_ALS_CONF_ALS_IT_800_ms | VEML7700_ALS_CONF_ALS_GAIN_x1,   // (1)
    VEML7700_ALS_CONF_ALS_IT_800_ms | VEML7700_ALS_CONF_ALS_GAIN_x1_4, // (2)
    VEML7700_ALS_CONF_ALS_IT_800_ms | VEML7700_ALS_CONF_ALS_GAIN_x1_8, // (3)
    VEML7700_ALS_CONF_ALS_IT_400_ms | VEML7700_ALS_CONF_ALS_GAIN_x1_8, // (4)
    VEML7700_ALS_CONF_ALS_IT_200_ms | VEML7700_ALS_CONF_ALS_GAIN_x1_8, // (5)
    VEML7700_ALS_CONF_ALS_IT_100_ms | VEML7700_ALS_CONF_ALS_GAIN_x1_8, // (6)
    VEML7700_ALS_CONF_ALS_IT_50_ms | VEML7700_ALS_CONF_ALS_GAIN_x1_8,  // (7)
    VEML7700_ALS_CONF_ALS_IT_25_ms | VEML7700_ALS_CONF_ALS_GAIN_x1_8,  // (8)
};
...
static int veml7700_reg_read(struct veml7700_data *drv_data, uint8_t reg, uint16_t *val)
{
	uint8_t valb[2];
	if (i2c_burst_read(drv_data->i2c, VEML7700_I2C_ADDRESS, reg, valb, 2))
	{
		return -EIO;
	}
...
static int veml7700_reg_write(struct veml7700_data *drv_data, uint8_t reg, uint16_t val)
{
	uint8_t bdata[2];
	bdata[0] = (uint8_t)(val & 0xFF);
	bdata[1] = (uint8_t)(val >> 8);

	return i2c_burst_write(drv_data->i2c, VEML7700_I2C_ADDRESS, reg, bdata, 2);
}

static int veml7700_reg_update(struct veml7700_data *drv_data, uint8_t reg, uint8_t mask, uint8_t val)
{
	uint16_t old_val = 0U;
	uint16_t new_val = 0U;

	if (veml7700_reg_read(drv_data, reg, &old_val) != 0)
	{
		return -EIO;
	}
	...
	return veml7700_reg_write(drv_data, reg, new_val);
}

static int veml7700_attr_set(const struct device *dev, enum sensor_channel chan,
			     enum sensor_attribute attr, const struct sensor_value *val)
{
	struct veml7700_data *drv_data = dev->data;

	if (veml7700_reg_update(drv_data, VEML7700_REG_ALS_CONF, VEML7700_ALS_CONF_ALS_SD_MASK, VEML7700_ALS_CONF_ALS_SD_ON))
	{
		return -EIO;
	}
...
static int veml7700_sample_fetch(const struct device *dev, enum sensor_channel chan)
{
	...
	if (veml7700_reg_read(drv_data, VEML7700_REG_ALS, &val))
	{
		return -EIO;
	}
	drv_data->sample = val;
...
static int veml7700_channel_get(const struct device *dev, enum sensor_channel chan, struct sensor_value *val)
{
	struct veml7700_data *drv_data = dev->data;
...
static int veml7700_power_update(struct veml7700_data *drv_data, uint16_t val)
{
	uint16_t old_config = 0U;
	uint16_t new_config = 0U;

	if (veml7700_reg_read(drv_data, VEML7700_REG_ALS_CONF, &old_config) != 0)
	{
		return -EIO;
	}
	//update params needed in every config update - here in power with existing config from sensors only
	uint16_t integration_bits = old_config & VEML7700_ALS_CONF_ALS_IT_MASK;
	uint16_t gain_bits = old_config & VEML7700_ALS_CONF_ALS_GAIN_MASK;
	...
	new_config = old_config & ~VEML7700_ALS_CONF_ALS_SD_MASK;
	new_config |= val & VEML7700_ALS_CONF_ALS_SD_MASK;

	return veml7700_reg_write(drv_data, VEML7700_REG_ALS_CONF, new_config);
}

static int veml7700_it_gain_update(struct veml7700_data *drv_data, uint16_t val)
{
	uint16_t old_config = 0U;
	uint16_t new_config = 0U;
	int ret = 0;
	...
	if (veml7700_reg_read(drv_data, VEML7700_REG_ALS_CONF, &old_config) != 0)
	{
		LOG_ERR("veml7700_reg_read() failed");
		return -EIO;
	}
	uint16_t mask = (VEML7700_ALS_CONF_ALS_IT_MASK | VEML7700_ALS_CONF_ALS_GAIN_MASK);
	new_config = old_config & ~mask;
	new_config |= val & mask;

	uint16_t integration_bits = new_config & VEML7700_ALS_CONF_ALS_IT_MASK;
	uint16_t gain_bits = new_config & VEML7700_ALS_CONF_ALS_GAIN_MASK;
	drv_data->config_it_gain = integration_bits | gain_bits;
	drv_data->integration_ms = lut_integration_ms[integration_bits >> 6];
	drv_data->gain = lut_gain[gain_bits >> 11];

	ret = veml7700_reg_write(drv_data, VEML7700_REG_ALS_CONF, new_config);
	if (ret)
	{
		LOG_ERR("veml7700_reg_write() failed");
	}
	return ret;
}

int veml7700_power_on(const struct device *dev)
{
	struct veml7700_data *drv_data = dev->data;
	int ret = veml7700_power_update(drv_data, VEML7700_ALS_CONF_ALS_SD_ON);
	if (ret)
	{
		LOG_ERR("veml7700_power_update() failed");
	}
	return ret;
}

int veml7700_power_off(const struct device *dev)
{
	struct veml7700_data *drv_data = dev->data;
	int ret = veml7700_power_update(drv_data, VEML7700_ALS_CONF_ALS_SD_OFF);
	if (ret)
	{
		LOG_ERR("veml7700_power_update() failed");
	}
...
int veml7700_init(const struct device *dev)
{
	struct veml7700_data *drv_data = dev->data;
	LOG_INF("veml7700_init()");
...
{
	struct veml7700_data *drv_data = dev->data;
	if (veml7700_sample_fetch(dev, SENSOR_CHAN_LIGHT) != 0)
	{
...
float veml7700_auto_measure(const struct device *dev)
{
	struct veml7700_data *drv_data = dev->data;
	float measure_lux = 0;

	veml7700_power_on(dev);	      //4 ms min after power on => will update the drv_data ->integration_ms and ->gain
	...
	while (!optimal_measure)
	{
		...
		measure_lux = veml7700_fetch_lux(dev);
